...
All three active simultaneously on every interaction:

### Layer 1: Zero-Width Unicode Fingerprint (Text)
- **Engine:** `server/zwc-fingerprint.ts` (encoder + decoder), `client/src/lib/zwc-fingerprint.ts` (client encoder)
- **Method:** Base-3 encoding using U+200B (zero-width space), U+200C (zero-width non-joiner), U+200D (zero-width joiner)
- **Behavior:** Copy button on every Clouud response. Client-side encodes conversationId + messageId + timestamp into invisible ZWC sequence, appends after visible text. Server decodes for verification.
- **Architecture:** Client encodes, server decodes. No round-trip on copy action.
- **Delimiter:** U+200B U+200C U+200D U+200B marks start/end of fingerprint

### Layer 2: Steganographic SVG Watermark (Images)
- **Engine:** `server/stego-watermark.ts`
- **Method:** Invisible micro-rect elements (0.5px, opacity 0.001) with width encoding bits (0.5=0, 0.6=1). Metadata tag with Base64 provenance as fast-path.
- **Behavior:** Auto-embedded in every SVG generated by `image-generator.ts`. Encodes session ID, founder ID, generation timestamp.
- **Extraction:** Metadata fast-path or stego fallback via micro-element position parsing.

### Layer 3: CSS Perceptual Hash Pattern (Screenshots)
- **Engine:** `client/src/lib/perceptual-hash-pattern.ts`
- **Method:** Micro-gradient stops at 0.001-0.003 opacity layered over the dynamic background. 32-bit hash mapped to gradient positions. Repeating 45-degree stripe as UUON marker.
- **Behavior:** Overlay div with pointer-events:none, mix-blend-mode:overlay, z-index above canvas. Regenerated per session. Invisible to naked eye, embedded in every screenshot.
- **Integration:** `client/src/components/dynamic-background.tsx` — provenance hash layer

### Unified Verification
- **Engine:** `server/provenance-verifier.ts`
- **Endpoints:**
  - `POST /api/provenance/decode-zwc` — decode ZWC fingerprint from text
  - `POST /api/provenance/verify-image` — extract watermark from SVG
  - `POST /api/provenance/verify-screenshot` — extract pattern hash from CSS gradient
  - `POST /api/provenance/verify` — unified endpoint, accepts any combination of `text`, `image`, `pattern`
- **Report:** Returns `{ zwcFingerprint, imageWatermark, cssPattern, methodsChecked, methodsFound, provenanceSummary }`
