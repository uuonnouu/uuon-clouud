...
export default function CLOUUDBrain(){
  ...
  const draw=useCallback(time=>{
    ...
    const sigColor=bullish?T:V; // teal for bull, violet for bear

    // ── Market-driven neural firing ──
    ...
    for(const i of nO){
      ...
      // color: base grey, energy shifts to teal (bull) or violet (bear)
    ...
    if(m.history.length>2){
      ...
      // dot
      const lx=hX+hW,ly=hY+hH-(((lastP.price-mn)/range)*hH);
  ...
  const sl=(label,key,min,max,step)=>(
    <div style={{marginBottom:6}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:1}}>
        ...
        <span style={{color:TEAL,fontSize:10}}>{params[key].toFixed(step<1?2:0)}</span>
      </div>
      <input type="range" min={min} max={max} step={step} value={params[key]}
        onChange={e=>set(key,parseFloat(e.target.value))}
        style={{width:"100%",accentColor:TEAL,height:2}}/>
    </div>
  );
  const btn={background:"rgba(4,8,15,0.94)",border:"1px solid #1a1a1a",color:"#666",padding:"4px 11px",cursor:"pointer",fontFamily:"'Share Tech Mono',monospace",fontSize:10,letterSpacing:1.5,backdropFilter:"blur(10px)"};
  ...
  return(
    <div style={{background:VOID,color:"#fff",width:"100%",height:"100vh",fontFamily:"'Share Tech Mono',monospace",position:"relative",overflow:"hidden"}}>
      ...
      <button onClick={()=>setSeed(s=>s+1)} style={{...btn,position:"absolute",top:12,right:panelOpen?252:70,zIndex:10,transition:"right .25s"}}>REGEN</button>

      {panelOpen&&(
        <div style={{position:"absolute",top:44,right:12,width:220,zIndex:10,background:"rgba(4,8,15,0.95)",border:"1px solid #152520",padding:"12px 12px 8px",overflowY:"auto",maxHeight:"calc(100vh - 56px)",backdropFilter:"blur(14px)"}}>
          <div style={{fontSize:9,color:TEAL,letterSpacing:2,marginBottom:8,opacity:0.6}}>MARKET DRIVE</div>
          {sl("fire rate","fireRate",0.01,1,0.01)}
          {sl("signal speed","signalSpeed",0.5,6,0.1)}
          {sl("decay","decay",0.8,1.0,0.002)}
          <div style={{borderTop:"1px solid #152520",margin:"8px 0",paddingTop:8}}>
            <div style={{fontSize:9,color:GOLD,letterSpacing:2,marginBottom:8,opacity:0.5}}>ΓH PARAMETERS</div>
          ...
          <div style={{borderTop:"1px solid #152520",margin:"8px 0",paddingTop:8}}>
            <div style={{fontSize:9,color:VIOLET,letterSpacing:2,marginBottom:8,opacity:0.5}}>STRUCTURE</div>
            {sl("trunks","trunks",4,24,1)}
            {sl("max depth","maxDepth",2,6,1)}
            {sl("branches","branchFactor",2,4,1)}
          </div>
          <div style={{borderTop:"1px solid #152520",margin:"8px 0",paddingTop:8}}>
            <div style={{fontSize:9,color:"#444",letterSpacing:2,marginBottom:8}}>ANIMATION</div>
            <button onClick={()=>set("animate",!params.animate)} style={{...btn,width:"100%",marginBottom:5,background:params.animate?"#0a1815":"transparent",color:params.animate?TEAL:"#444",borderColor:params.animate?"#1e3a35":"#1a1a1a"}}>{params.animate?"ACTIVE":"PAUSED"}</button>
            {params.animate&&sl("pulse speed","pulseSpeed",0.1,4,0.1)}
          </div>
        </div>
      )}
    </div>
