...
const DEFAULT_MODEL = process.env.OLLAMA_MODEL ?? 'clouud';
...
async function* streamOllama(prompt: string, model: string): AsyncGenerator<string> {
...
export const OllamaGateway = {
  async *ask(question: string, opts: AskOptions = {}): AsyncGenerator<string> {
    ...
    yield* streamOllama(await buildPrompt(question, ctx), model);
  },
