...
export { d13mon4HashEngine, D13MON4HashEngine } from './d13mon4HashEngine';
...
export class CryptoSuite {
  ...
  async generateD13MON4Hash(inputText: string) {
    const { d13mon4HashEngine } = await import('./d13mon4HashEngine');
    return await d13mon4HashEngine.generateHash(inputText);
  }

  async generateGeometricLatticeTokens(latitude: number, longitude: number, dimensionalOffset: number = 0) {
    const { d13mon4HashEngine } = await import('./d13mon4HashEngine');
    return await d13mon4HashEngine.generateLatticeTokens(latitude, longitude, dimensionalOffset);
  }
}
