...
- [Drizzle neon-http null bug](drizzle-neon-http-null-bug.md) — drizzle-orm neon-http driver returns null (not []) for 0-row SELECT and for INSERT...RETURNING; use pg.Pool directly for all auth DB ops.
- [Session secure-cookie HTTP gap](session-secure-cookie-http.md) — Replit sets NODE_ENV=production so session cookies are secure=true; curl HTTP tests never receive Set-Cookie, but browser HTTPS flow works correctly.
- [Build cache source hash](build-cache-source-hash.md) — start.sh caches build in dist/.source_hash; delete that file to force a rebuild when esbuild doesn't pick up changes.
