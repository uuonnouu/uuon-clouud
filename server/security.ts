...
import { getValidSession, isSetupComplete } from "./auth";
...
async function enforceFullAuth(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!(fp && fp.isOwner)) {
      ...
      const setupDone = setup.webauthn && setup.passphrase && setup.fingerprint;
      if (!setupDone) {
        const SETUP_ALLOWED = [
          "/api/conversations",
          "/api/metrics",
        ...
        const isSetupAllowed = SETUP_ALLOWED.some(p => req.path === p || req.path.startsWith(p + "/"));
        if (isSetupAllowed) {
          (req as any).isOwner = true;
          return next();
        }
